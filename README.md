...
4. 메뉴에서 `New File` 을 클릭하여 파이썬 코드를 작성할 수 있는 새 창을 띄운다. (단축키: `Ctrl` + `N`)
5. 메뉴에서 `Save` 를 클릭하고 작성한 파이썬 코드를 저장한다. (단축키: `Ctrl` + `S`)
6. 파이썬 코드를 작성한 뒤에 메뉴에서 `Run` > `Run Module` 을 클릭한다. (단축키: `F5`)
